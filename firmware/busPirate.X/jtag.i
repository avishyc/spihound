...
static uint8_t jtag_write_bit(const _Bool value);
...
void jtag(void) {
  jtag_setup();
  jtag_settings.hiz = 1;
  jtag_settings.bit_pending = 0;
  jtag_settings.delayed_bit = 0;
...
uint8_t jtag_write_bit(const _Bool value) {
  if (value == 0) {
    jtag_data_low();
  } else {
    jtag_data_high();
  ...
  jtag_clock_high();
  uint8_t input_bit = PORTDbits.RD3;
  jtag_clock_low();
  return input_bit;
}
...
void jtag_data_high(void) {
  TRISDbits.TRISD1 = ~jtag_settings.hiz;
  PORTDbits.RD1 = jtag_settings.hiz;
  bp_delay_us(20);
}
...
void jtag_clock_high(void) {
  TRISDbits.TRISD2 = ~jtag_settings.hiz;
  PORTDbits.RD2 = jtag_settings.hiz;
  bp_delay_us(100);
}
...
void jtag_tms_high(void) {
  TRISDbits.TRISD4 = ~jtag_settings.hiz;
  PORTDbits.RD4 = jtag_settings.hiz;
  bp_delay_us(20);
}
