...
_Bool bp_binary_io_pullup_control(uint8_t control_byte);
# 59 "../1wire.c" 2
# 1 "../proc_menu.h" 1
# 25 "../proc_menu.h"
...
# 467 "../1wire.c"
static _Bool onewire_internal_bit_io(_Bool bit_value);
# 483 "../1wire.c"
...
# 658 "../1wire.c"
static _Bool device_find_first(void);
# 668 "../1wire.c"
static _Bool device_find_next(void);
# 681 "../1wire.c"
static _Bool perform_device_search(void);
# 691 "../1wire.c"
static void print_device_information(const size_t roster_id,
                                     const uint8_t *rom_address);
# 701 "../1wire.c"
static inline void onewire_internal_set_data_state(const _Bool state);

uint16_t onewire_read(void) { return onewire_internal_byte_io(0xFF); }

uint16_t onewire_write(const uint16_t value) {
  do { onewire_internal_byte_io(value & 0xFF); } while (0);

  return 0x100;
}

_Bool onewire_read_bit(void) { return onewire_internal_bit_io(1); }

void onewire_clock_pulse(void) { do { onewire_internal_bit_io(onewire_state.data_state); } while (0); }

uint16_t onewire_data_state(void) { return onewire_state.data_state; }

inline void onewire_data_low(void) { onewire_internal_set_data_state(0); }

inline void onewire_data_high(void) { onewire_internal_set_data_state(1); }
...
void onewire_setup_prepare(void) {
  ...
  if ((speed > 0) && (speed <= 2)) {
    ...
    bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_SPEED_PROMPT_str));
...
void print_device_information(const size_t roster_id,
  ...
  bp_message_write_buffer(__builtin_tbladdress(MSG_1WIRE_LOOKUP_ID_HEADER_str));
...
void onewire_run_macro(const uint16_t macro) {
  ...
  if ((macro_id > 0) && (macro_id < 50)) {
    ...
    if (macro_id >= onewire_state.used_roster_entries) {
      ...
      bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_NO_DEVICE_str));
      return;
    }
    ...
    bp_message_write_buffer(__builtin_tbladdress(MSG_1WIRE_ADDRESS_MACRO_HEADER_str));
    bp_write_dec_byte(macro + 1);
    bp_write_string(": ");
    for (rom_index = 0; rom_index < 8; rom_index++) {
      bp_write_formatted_integer(
          onewire_state.roster_entries[macro_id][rom_index]);
      user_serial_transmit_character(' ');
      do { onewire_internal_byte_io(onewire_state.roster_entries[macro_id][rom_index]); } while (0);
    }
    bp_write_line("");
    return;
  }
  ...
  case MACRO_DUMP_ROSTER: {
    ...
    bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_MACRO_MENU_HEADER_str));
    bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_MACRO_TABLE_HEADER_str));
    ...
    bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_MACRO_LIST_str));
    break;
  }
  ...
  case MACRO_SEARCH_ROM: {
    ...
    if (macro_id == MACRO_ALARM_SEARCH) {
      bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_ALARM_MACRO_NAME_str));
    } else {
      bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_SEARCH_MACRO_NAME_str));
    }
    ...
    bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_MACRO_TABLE_TRAILER_str));
    break;
  }
  ...
  case MACRO_READ_ROM: {
    ...
    onewire_reset();
    bp_message_write_buffer(__builtin_tbladdress(MSG_1WIRE_READ_ROM_MACRO_NAME_str));
    do { onewire_internal_byte_io(MACRO_READ_ROM); } while (0);
    for (index = 0; index < 8; index++) {
      device_address[index] = onewire_internal_byte_io(0xFF);
  ...
  case MACRO_MATCH_ROM:
    onewire_reset();
    bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_MATCH_ROM_MACRO_NAME_str));
    do { onewire_internal_byte_io(MACRO_MATCH_ROM); } while (0);
    break;

  case MACRO_SKIP_ROM:
    onewire_reset();
    bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_SKIP_ROM_MACRO_NAME_str));
    do { onewire_internal_byte_io(MACRO_SKIP_ROM); } while (0);
    break;

  default:
    bp_message_write_line(__builtin_tbladdress(MSG_UNKNOWN_MACRO_ERROR_str));
    break;
  }
}

void onewire_pins_state(void) { bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_PINS_STATE_str)); }

void onewire_reset(void) {
  onewire_bus_reset_result_t reset_result;

  reset_result = perform_bus_reset();
  bp_message_write_buffer(__builtin_tbladdress(MSG_1WIRE_BUS_RESET_str));

  if (reset_result == ONEWIRE_BUS_RESET_OK) {
    bp_message_write_line(__builtin_tbladdress(BPMSG1185_str));
    return;
  }

  bp_message_write_buffer(__builtin_tbladdress(MSG_WARNING_HEADER_str));
  if (reset_result == ONEWIRE_BUS_RESET_SHORT) {
    bp_message_write_buffer(__builtin_tbladdress(MSG_WARNING_SHORT_OR_NO_PULLUP_str));
  } else {
    bp_message_write_buffer(__builtin_tbladdress(MSG_1WIRE_NO_DEVICE_DETECTED_str));
  }
...
void lookup_device_model(const uint8_t model) {
  switch (model) {
  case 0x10:
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1022_str));
    break;

  case 0x28:
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1023_str));
    break;

  case 0x22:
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1024_str));
    break;

  case 0x04:
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1025_str));
    break;

  case 0x2D:
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1026_str));
    break;

  default:
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1027_str));
  }
...
_Bool device_find_first() {
  onewire_state.last_device_discrepancy = 0;
  onewire_state.last_family_discrepancy = 0;
  onewire_state.last_device_flag = 0;

  return perform_device_search();
}

_Bool device_find_next(void) { return perform_device_search(); }

_Bool perform_device_search(void) {
  _Bool id_bit;
  _Bool cmp_id_bit;
  ...
  if (!onewire_state.last_device_flag) {
    ...
    do { onewire_internal_byte_io(onewire_state.command_byte); } while (0);

    do {



      id_bit = onewire_internal_bit_io(1);
      cmp_id_bit = onewire_internal_bit_io(1);
      ...
      do { onewire_internal_bit_io(search_direction); } while (0);
    ...
    if (!((id_bit_number < 65) || (onewire_state.crc8 != 0))) {
      ...
      if (onewire_state.last_device_discrepancy == 0) {
        onewire_state.last_device_flag = 1;
      }
...
void binary_io_enter_1wire_mode(void) {
  ...
  bp_message_write_buffer(__builtin_tbladdress(MSG_1WIRE_MODE_IDENTIFIER_str));
  ...
  for (;;) {
    ...
    case 0x00:
      ...
      case 0x04:
        user_serial_transmit_character(onewire_internal_byte_io(0xFF));
        break;
    ...
    case 0x01: {
      ...
      for (index = 0; index < input_byte; index++) {
        do { onewire_internal_byte_io(user_serial_read_byte()); } while (0);
        do { user_serial_transmit_character(0x01); } while (0);
      }
...
_Bool onewire_internal_bit_io(_Bool bit_value) {
  TRISDbits.TRISD1 = 1;
  PORTDbits.RD1 = 0;
  TRISDbits.TRISD1 = 0;
...
void onewire_internal_set_data_state(const _Bool state) {
  onewire_state.data_state = state;
  bp_message_write_line(__builtin_tbladdress(MSG_1WIRE_NEXT_CLOCK_ALERT_str));
}
