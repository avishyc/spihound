...
# 20 "../dp_usb/usb_stack.c" 2
# 30 "../dp_usb/usb_stack.c"
...
usb_ep_t endpoints[(16u)];
# 68 "../dp_usb/usb_stack.c"
#pragma udata usb_bdt
BDentry usb_bdt[2 + 2 * 2u] __attribute__((aligned(512)));

BYTE usb_ep0_out_buf[8u];
BYTE usb_ep0_in_buf[8u];
...
void usb_init( const BYTE *device_descriptor,
    ...
    sof_handler = (0);
    class_setup_handler = (0);
    vendor_setup_handler = (0);
    usb_set_in_handler(0, (usb_handler_t) 0);
    usb_set_out_handler(0, (usb_handler_t) 0);
    ClearUSBtoDefault();
    do { U1CNFG1 = 0; U1CNFG2 = (0); U1BDTP1 = (unsigned int) usb_bdt/256; U1OTGCON = (1<<7) | (0); U1PWRCbits.USBPWR = 1; } while(0);
    U1CONbits.PKTDIS = 0;
}
...
void ClearUSBtoDefault(void) {
    ...
    (U1ADDR = (0));
    do {U1CONbits.PPBRST = 1; U1CONbits.PPBRST=0;} while(0);
    U1EIR = 0xFF;

    for (i = 0; i < (16u); i++) {
        endpoints[i].out_handler = (0);
        endpoints[i].in_handler = (0);
        ((usb_uep_t*)(&U1EP0))[i] = 0;
    }
    ...
    U1EP0 = ((0x01) | (0x04) | (0x08));
# 166 "../dp_usb/usb_stack.c"
    ...
    usb_bdt[( 2*(0)+(0) )].BDCNT = 8u;
    usb_bdt[( 2*(0)+(0) )].BDADDR = usb_ep0_out_buf;
    usb_bdt[( 2*(0)+(0) )].BDSTAT = 0x80 + 0x08;
    usb_bdt[( 2*(0)+(1) )].BDCNT = 0;
    usb_bdt[( 2*(0)+(1) )].BDADDR = usb_ep0_in_buf;
    usb_bdt[( 2*(0)+(1) )].BDSTAT = 0x40 + 0x08;
...
void usb_handle_transaction(void) {
    ...
    trn_status = (U1STAT);
    EP0_Outbdp = &usb_bdt[( (trn_status)/8 )];
    EP0_Inbdp = &usb_bdt[( (trn_status | 0x8)/8 )];

    switch (EP0_Outbdp->BDSTAT & 0b00111100) {
        case 0b00110100:
            usb_handle_setup();
            break;
        case 0b00000100:
...
void usb_handle_setup(void) {
    ...
    EP0_Inbdp->BDSTAT = 0x08;

    U1CONbits.PKTDIS = 0;

    switch (EP0_Outbdp->BDADDR[0] & 0b01100000) {
        case 0b00000000:
            switch (EP0_Outbdp->BDADDR[0] & 0b00000011) {
                case 0b00000000:
    ...
    EP0_Outbdp->BDSTAT = (!(EP0_Outbdp->BDADDR[0] & 0b10000000) &&
            (EP0_Outbdp->BDADDR[6] || EP0_Outbdp->BDADDR[7])) ? 0x80 + 0x40 + 0x08 : 0x80 + 0x08;
}
...
void usb_handle_StandardDeviceRequest(BDentry *bdp) {
    ...
    switch (packet[1]) {
        ...
        case 3:
            ...
            break;
        case 5:
            if (0x00u == packet[3] && 0x7Fu >= packet[2]) {
                usb_addr_pending = packet[2];
        ...
        case 6:
            switch (packet[3]) {
                case 1u:
                    ...
                    if ((0 == packet[7] && packet[6] < usb_rom_ptr[0]))
                        usb_rom_len = packet[6];
                    break;

                case 2u:
                    if (packet[2] >= usb_device_descriptor[17]) {
                        usbrequesterrorflag = 1;
                        break;
                    }
                    ...
                    if ((packet[7] < usb_rom_ptr[3]) ||
                            (packet[7] == usb_rom_ptr[3] && packet[6] < usb_rom_ptr[2]))
                        usb_rom_len = packet[6] + packet[7] * 256;
                    break;
                case 3u:

                    if (packet[2] >= usb_num_string_descriptors) {
                        usbrequesterrorflag = 1;
                        break;
                    }
    ...
    switch (packet[1]) {
        ...
        case 1:
            ...
            *pUEP &= ~(0x02);
            ...
            epbd = &usb_bdt[( 2*(epnum)+(dir) )];
            epbd->BDSTAT &= ~0x04;
            if (dir) epbd->BDSTAT |= 0x40;
            if (0 == dir) epbd->BDSTAT &= ~0x40;
# 490 "../dp_usb/usb_stack.c"
            usb_ack_dat1(0);
            break;
...
void usb_handle_in(void) {
    if (endpoints[((trn_status>>4)&0x0F)].in_handler) {
        endpoints[((trn_status>>4)&0x0F)].in_handler();
    }
}

void usb_handle_out(void) {
    if (endpoints[((trn_status>>4)&0x0F)].out_handler) {
        endpoints[((trn_status>>4)&0x0F)].out_handler();
    }
}
...
void usb_ack_dat1(int bdcnt) {
    ...
    EP0_Inbdp->BDSTAT = (0x40 | 0x80 | 0x08);
}
...
void usb_RequestError(void) {
    ...
    usb_bdt[( 2*(0)+(1) )].BDSTAT = 0x80 + 0x04;
    usb_bdt[( 2*(0)+(0) )].BDSTAT = 0x80 + 0x04;
...
void usb_send_rom(void) {
    ...
    size_t packet_len;
    if (usb_rom_len) {
        packet_len = (usb_rom_len < 8u) ? usb_rom_len : 8u;
    ...
    EP0_Inbdp->BDSTAT = ((EP0_Inbdp->BDSTAT ^ 0x40) & 0x40) | 0x80 | 0x08;
