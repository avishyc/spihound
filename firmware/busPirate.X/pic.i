...
void pic_setup_prepare(void) {
  ...
  if (interactive) {
    ...
    bp_message_write_line(__builtin_tbladdress(MSG_PIC_MODE_PROMPT_str));
    mode_info.opcode_bits = getnumber(1, 1, 2, 0) - 1;
    bp_message_write_line(__builtin_tbladdress(MSG_PIC_DELAY_PROMPT_str));
    mode_info.delay = getnumber(1, 0, 3, 0);
  } else {
    bp_message_write_buffer(__builtin_tbladdress(MSG_PIC_MODE_HEADER_str));
  ...
  bitbang_set_pins_low(0b0000000000000010 | 0b0000000000000100, 100);
}

void pic_setup_execute(void) { mode_configuration.int16 = 1; }

void pic_cleanup(void) { mode_configuration.int16 = 0; }

void pic_start(void) {
  mode_info.mode = 1;
  bp_message_write_buffer(__builtin_tbladdress(MSG_PIC_MODE_COMMAND_str));
...
void pic_stop(void) {
  mode_info.mode = 0;
  mode_configuration.int16 = 1;
  bp_message_write_line(__builtin_tbladdress(MSG_PIC_MODE_DATA_str));
}
...
uint16_t read_bits(const uint8_t bits) {
  ...
  for (size_t index = 0; index < bits; index++) {
    bitbang_set_pins_high(0b0000000000000100, 100 / 2);
    data = (data >> 1) |
           ((bitbang_read_pin(0b0000000000000010) == 1 ? 1 : 0) << (1 << (bits - 1)));
    bitbang_set_pins_low(0b0000000000000100, 100 / 2);
  }

  bitbang_set_pins_high(0b0000000000000100, 100 / 2);
  bitbang_set_pins_low(0b0000000000000100, 100 / 2);
  bitbang_set_pins_low(0b0000000000000010, 100 / 5);
...
unsigned int pic_read(void) {
  if (mode_info.mode == 1) {
    bp_message_write_line(__builtin_tbladdress(MSG_PIC_NO_READ_str));
    return 0;
  }

  switch (mode_info.opcode_bits) {
  case PIC_MODE_6BITS:
    bitbang_read_pin(0b0000000000000010);
    bitbang_set_pins_high(0b0000000000000100, 100 / 2);
    bitbang_set_pins_low(0b0000000000000100, 100 / 2);
    return read_bits(14);
  ...
  default:
    bp_message_write_line(__builtin_tbladdress(MSG_PIC_UNKNOWN_MODE_str));
    return 0;
  }
}
...
void write_bits(const uint16_t data, const uint8_t bits) {
  ...
  for (size_t index = 0; index < bits; index++) {
    bitbang_set_pins_high(0b0000000000000100, 100 / 4);
    bitbang_set_pins((data & mask) == mask, 0b0000000000000010, 100 / 4);
    bitbang_set_pins_low(0b0000000000000100, 100 / 4);
    bitbang_set_pins_low(0b0000000000000010, 100 / 4);
    mask <<= 1;
  }
}
...
void pic_run_macro(const uint16_t macro) {
  switch (macro) {
  case 0:
    bp_message_write_line(__builtin_tbladdress(MSG_PIC_MACRO_MENU_str));
    break;
  ...
  case 1:
    ...
    case PIC_MODE_6BITS: {
      ...
      bp_message_write_buffer(__builtin_tbladdress(MSG_PIC_DEVICE_ID_str));
      bp_write_hex_word(temp >> 5);
      bp_message_write_buffer(__builtin_tbladdress(MSG_PIC_REVISION_ID_str));
    ...
    case PIC_MODE_4BITS:
    default:
      bp_message_write_line(__builtin_tbladdress(MSG_PIC_MACRO_NOT_IMPLEMENTED_str));
    }

    bp_message_write_line(__builtin_tbladdress(MSG_PIC_EXIT_MODE_str));
    break;
...
void pic_print_pins_state(void) { bp_message_write_line(__builtin_tbladdress(MSG_PIC_PINS_STATE_str)); }
# 342 "../pic.c"
void binary_io_enter_pic_mode(void) {
  bp_message_write_buffer(__builtin_tbladdress(MSG_PIC_MODE_IDENTIFIER_str));
  ...
  for (;;) {
    ...
    case 0x00:
      ...
      case 0x10:
        if (command & 0x08) {
          bitbang_set_pins((command & 0x04) == 0x04, 0b0000000000100000, 5);
          bitbang_set_pins((command & 0x02) == 0x02, 0b0000000000001000, 5);
          bitbang_set_pins((command & 0x01) == 0x01, 0b0000000000010000, 5);
        } else {
          if (command & 0x04) {
      ...
      user_serial_transmit_character(ok != 0 ? 0x01
                                             : 0x00);
      break;
