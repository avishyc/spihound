...
# 139 "../spi.c"
static void spi_sniffer(_Bool trigger, _Bool terminal_mode);
...
static void engage_spi_cs(_Bool write_with_read);
# 156 "../spi.c"
static void spi_read_write_io(const _Bool engage_cs);
...
void engage_spi_cs(_Bool write_with_read) {
  mode_configuration.write_with_read = write_with_read;
  PORTDbits.RD4 = !spi_state.cs_line_state;
  if (spi_state.cs_line_state) {
    user_serial_transmit_character('/');
  }
  bp_message_write_line(__builtin_tbladdress(MSG_SPI_CS_ENABLED_str));
}

inline void spi_start_with_read(void) { engage_spi_cs(1); }

inline void spi_start(void) { engage_spi_cs(0); }
...
void spi_stop(void) {
  ...
  bp_message_write_line(__builtin_tbladdress(MSG_SPI_CS_DISABLED_str));
}
...
void spi_print_settings(void) {
  bp_message_write_buffer(__builtin_tbladdress(MSG_SPI_MODE_HEADER_START_str));
  ...
  bp_message_write_line(__builtin_tbladdress(MSG_MODE_HEADER_END_str));
}
...
void spi_setup_prepare(void) {
  ...
  if (user_prompt) {
    ...
    bp_message_write_line(__builtin_tbladdress(MSG_SPI_SPEED_PROMPT_str));
    mode_configuration.speed = getnumber(1, 1, 12, 0) - 1;

    bp_message_write_line(__builtin_tbladdress(MSG_SPI_POLARITY_PROMPT_str));
    spi_state.clock_polarity = getnumber(1, 1, 2, 0) - 1;

    bp_message_write_line(__builtin_tbladdress(MSG_SPI_EDGE_PROMPT_str));
    spi_state.clock_edge = getnumber(2, 1, 2, 0) - 1;

    bp_message_write_line(__builtin_tbladdress(MSG_SPI_SAMPLE_PROMPT_str));
    spi_state.data_sample_timing = getnumber(1, 1, 2, 0) - 1;

    bp_message_write_line(__builtin_tbladdress(MSG_SPI_CS_MODE_PROMPT_str));
    spi_state.cs_line_state = getnumber(2, 1, 2, 0) - 1;

    bp_message_write_line(__builtin_tbladdress(MSG_PIN_OUTPUT_TYPE_PROMPT_str));
...
void spi_run_macro(const uint16_t macro) {
  ...
  switch (macro) {
  case SPI_MACRO_MENU:
    bp_message_write_line(__builtin_tbladdress(MSG_SPI_MACRO_MENU_str));
    break;

  case SPI_MACRO_SNIFF_ON_CS_LOW:
    bp_message_write_line(__builtin_tbladdress(MSG_SNIFFER_MESSAGE_str));
    bp_message_write_line(__builtin_tbladdress(MSG_ANY_KEY_TO_EXIT_PROMPT_str));
    spi_sniffer(0, 1);
    break;
...
void spi_print_pins_state(void) { bp_message_write_line(__builtin_tbladdress(MSG_SPI_PINS_STATE_str)); }
...
# 486 "../spi.c"
  SPI1CON1 =
      ((((uint8_t)spi_speed) & ((1 << (uint8_t)5) - 1)) << 0x00000000) |
      (1 << 0x00000005) |
      ((((uint8_t)spi_state.clock_polarity) & ((1 << (uint8_t)1) - 1)) << 0x00000006) |
      ((((uint8_t)spi_state.clock_edge) & ((1 << (uint8_t)1) - 1)) << 0x00000008) |
      ((((uint8_t)spi_state.data_sample_timing) & ((1 << (uint8_t)1) - 1)) << 0x00000009);
# 504 "../spi.c"
  SPI1CON2 = 0x0000;
# 516 "../spi.c"
  SPI1STAT = 0x00008000;
...
void spi_disable_interface(void) {
  ...
  RPOR12bits.RP24R = 0b00000;
  RPOR11bits.RP23R = 0b00000;
...
void spi_sniffer(_Bool trigger, _Bool terminal_mode) {
  _Bool last_cs_line_state;

restart:

  last_cs_line_state = PORTDbits.RD4;
  ...
  for (;;) {
    _Bool data_pending = SPI1STATbits.SRXMPT == 0 && SPI2STATbits.SRXMPT == 0;


    if (data_pending == 0 && last_cs_line_state == 0 && PORTDbits.RD4 == 1) {
      user_serial_ringbuffer_append(']');
      last_cs_line_state = 1;
    }
    else if (last_cs_line_state == 1 && PORTDbits.RD4 == 0) {
      user_serial_ringbuffer_append('[');
      last_cs_line_state = 0;
    }
    ...
    if ((SPI1STATbits.SPIROV == 1) || (SPI2STATbits.SPIROV == 1) ||
        (bus_pirate_configuration.overflow == 1)) {


      if (bus_pirate_configuration.overflow == 0) {
...
# 639 "../spi.c"
      ...
      if (terminal_mode) {
        bp_message_write_line(__builtin_tbladdress(MSG_SPI_COULD_NOT_KEEP_UP_str));
        goto restart;
      }
...
void spi_slave_enable(void) {
  ...
  RPINR21bits.SS1R = 25;
  RPINR23bits.SS2R = 25;
  RPINR20bits.SDI1R = 24;
  RPINR20bits.SCK1R = 23;
  RPINR22bits.SDI2R = 22;
  RPINR22bits.SCK2R = 23;
# 702 "../spi.c"
  SPI1CON1 =
      ((((uint8_t)spi_bus_speed[mode_configuration.speed]) & ((1 << (uint8_t)5) - 1))
       << 0x00000000) |
      ((((uint8_t)spi_state.clock_polarity) & ((1 << (uint8_t)1) - 1)) << 0x00000006) |
      ((((uint8_t)spi_state.clock_edge) & ((1 << (uint8_t)1) - 1)) << 0x00000008);
# 719 "../spi.c"
...
void spi_enter_binary_io(void) {
  ...
  spi_setup(spi_bus_speed[mode_configuration.speed]);
  bp_message_write_buffer(__builtin_tbladdress(MSG_SPI_MODE_IDENTIFIER_str));
  ...
  for (;;) {
    ...
    case SPI_COMMAND_CONFIGURE_SPI:
      ...
      spi_state.data_sample_timing = (input_byte & 0b0001)
                                         ? 1
                                         : 0;
      mode_configuration.high_impedance = (input_byte & 0b1000) == 0 ? 1 : 0;
      spi_setup(spi_bus_speed[mode_configuration.speed]);
      do { user_serial_transmit_character(0x01); } while (0);
      break;
...
void spi_read_write_io(const _Bool engage_cs) {
...
void handle_extended_avr_command(void) {
  ...
  case 1:
    do { user_serial_transmit_character(0x01); } while (0);
    user_serial_transmit_character((((uint16_t)(0x0001) >> 8) & 0xFF));
    user_serial_transmit_character(((uint16_t)(0x0001)&0xFF));
    break;
