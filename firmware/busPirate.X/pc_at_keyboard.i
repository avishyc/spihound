...
# 1 "../pc_at_keyboard.c"
# 18 "../pc_at_keyboard.c"
# 1 "../pc_at_keyboard.h" 1
# 21 "../pc_at_keyboard.h"
# 1 "../configuration.h" 1
# 22 "../pc_at_keyboard.h" 2
...
static _Bool keyboard_wait_clock_change(const _Bool expected);
...
static _Bool write_bit(const _Bool value);
static void handle_scancode(const keyboard_scancode_read_result_t result);

void pc_at_keyboard_prepare(void) { mode_configuration.high_impedance = 1; }
...
uint16_t pc_at_keyboard_send(const uint16_t value) {
  ...
  default:
    bp_message_write_buffer(__builtin_tbladdress(MSG_KEYBOARD_ERROR_TIMEOUT_str));
    break;
  }
...
void pc_at_keyboard_run_macro(const uint16_t macro) {
  ...
  case KEYBOARD_MACRO_MENU:
    bp_message_write_line(__builtin_tbladdress(MSG_KEYBOARD_MACRO_MENU_str));
    break;

  case KEYBOARD_MACRO_LIVE_INPUT_MONITOR:
    bp_message_write_line(__builtin_tbladdress(MSG_KEYBOARD_LIVE_INPUT_START_str));
...
void handle_scancode(const keyboard_scancode_read_result_t result) {
  ...
  case KEYBOARD_SCANCODE_READ_START_BIT_ERROR:
    bp_message_write_line(__builtin_tbladdress(MSG_KEYBOARD_ERROR_STARTBIT_str));
    break;

  case KEYBOARD_SCANCODE_READ_PARITY_ERROR:
    bp_message_write_line(__builtin_tbladdress(MSG_KEYBOARD_ERROR_PARITY_str));
    break;

  case KEYBOARD_SCANCODE_READ_STOP_BIT_ERROR:
    bp_message_write_line(__builtin_tbladdress(MSG_KEYBOARD_ERROR_STOPBIT_str));
    break;
  ...
  case KEYBOARD_SCANCODE_READ_NO_DATA:
    bp_message_write_line(__builtin_tbladdress(MSG_KEYBOARD_ERROR_NODATA_str));
    break;

  default:
    bp_message_write_line(__builtin_tbladdress(MSG_KEYBOARD_ERROR_UNKNOWN_str));
    break;
  }
}
...
keyboard_read_result_t read_bit(void) {
  ...
  if (keyboard_wait_clock_change(0) == 1) {
    return KEYBOARD_READ_TIMEOUT;
  }

  result = (keyboard_read_result_t)PORTDbits.RD1;

  if (keyboard_wait_clock_change(1) == 1) {
    return KEYBOARD_READ_TIMEOUT;
  }
...
_Bool write_bit(const _Bool value) {
  if (keyboard_wait_clock_change(1) == 1) {
    return 1;
  }
...
keyboard_write_byte_result_t write_byte(const uint8_t value) {
  ...
  if (write_bit((((uint8_t)parity) & ((1 << (uint8_t)1) - 1)) != 0)) {
    return KEYBOARD_WRITE_BYTE_TIMEOUT;
  }
...
_Bool keyboard_wait_clock_change(const _Bool expected) {
  volatile uint16_t delay = 0xFFFF;

  while (delay > 0) {
    if (PORTDbits.RD2 == expected) {
      return 0;
    }
