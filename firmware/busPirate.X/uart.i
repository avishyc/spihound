...
# 85 "../uart.c"
static uint32_t uart_get_baud_rate(const _Bool quiet);
...
uint16_t uart_read(void) {
  if (uart2_rx_ready()) {
    ...
    if (U2STAbits.PERR) {
      bp_message_write_buffer(__builtin_tbladdress(MSG_UART_PARITY_ERROR_str));
    }


    if (U2STAbits.FERR) {
      bp_message_write_buffer(__builtin_tbladdress(MSG_UART_FRAMING_ERROR_str));
    }
    ...
    if (U2STAbits.OERR) {
      bp_message_write_buffer(__builtin_tbladdress(MSG_UART_OVERRUN_ERROR_str));
  ...
  bp_message_write_buffer(__builtin_tbladdress(MSG_UART_FAILED_NO_DATA_str));
  return 0;
}
...
void UARTsettings(void) {
  bp_message_write_buffer(__builtin_tbladdress(MSG_UART_MODE_HEADER_str));
...
void uart_setup_prepare(void) {
  ...
  if (speed == 0) {
    ...
    mode_configuration.speed = getnumber(1, 1, 11, 0) - 1;

    if (mode_configuration.speed == 10) {
      mode_configuration.speed = 8;
      abd = 1;
      uart_settings.autobaud = 1;
      bp_message_write_line(__builtin_tbladdress(MSG_BAUD_DETECTION_SELECTED_str));
    }

    if (mode_configuration.speed == 9) {
      bp_message_write_line(__builtin_tbladdress(MSG_UART_CUSTOM_BAUD_RATE_PROMPT_str));
      abd = getlong(115200, 1, 999999, 0);
...
# 237 "../uart.c"
    bp_message_write_buffer(__builtin_tbladdress(MSG_UART_BITS_PARITY_PROMPT_str));
    uart_settings.databits_parity = getnumber(1, 1, 4, 0) - 1;

    bp_message_write_buffer(__builtin_tbladdress(MSG_UART_BITS_STOP_PROMPT_str));
    uart_settings.stop_bits = getnumber(1, 1, 2, 0) - 1;

    bp_message_write_buffer(__builtin_tbladdress(MSG_UART_POLARITY_PROMPT_str));
...
void uart_setup_execute(void) {
  ...
  unsigned long abd;
  int brg;
  if (uart_settings.autobaud == 1) {
    uart2_disable();
    bp_write_line("");
    abd = uart_get_closest_common_rate(uart_get_baud_rate(0));
    bp_write_line("");
...
void uart_run_macro(const uint16_t macro) {
  switch (macro) {
  case 0:
    bp_message_write_buffer(__builtin_tbladdress(MSG_UART_MACRO_MENU_str));
    break;

  case 3:
# 328 "../uart.c"
  case 1:
    bp_message_write_line(__builtin_tbladdress(MSG_UART_BRIDGE_str));
    bp_message_write_line(__builtin_tbladdress(MSG_UART_BRIDGE_EXIT_str));
...
# 380 "../uart.c"
  ...
  case 2:
    bp_message_write_line(__builtin_tbladdress(MSG_UART_RAW_UART_INPUT_str));
...
void uart_start(void) {
  ...
  uart_settings.echo_uart = 1;


  mode_configuration.periodicService = 1;

  bp_message_write_line(__builtin_tbladdress(MSG_UART_LIVE_DISPLAY_START_str));
}

void uart_stop(void) {

  uart_settings.echo_uart = 0;


  mode_configuration.periodicService = 0;

  bp_message_write_line(__builtin_tbladdress(MSG_UART_LIVE_DISPLAY_STOP_str));
}
...
inline void uart_pins_state(void) { bp_message_write_line(__builtin_tbladdress(MSG_UART_PINS_STATE_str)); }
...
uint32_t uart_get_baud_rate(const _Bool quiet) {
  size_t samples;
  ...
  if (!quiet) {
    bp_message_write_line(__builtin_tbladdress(MSG_UART_WAITING_ACTIVITY_str));
  }


  while ((PORTDbits.RD3 == 1) && (U1STAbits.URXDA == 0)) {
    __builtin_nop();
  }
...
# 579 "../uart.c"
    ...
    if (!quiet) {
      bp_message_write_line(__builtin_tbladdress(MSG_UART_EARLY_EXIT_str));
    }
...
# 642 "../uart.c"
  ...
  bit_sample = 16000000UL / bit_sample;

  if (!quiet) {
    if (bit_sample > 150000) {
      bp_message_write_line(__builtin_tbladdress(MSG_UART_BAUD_OVERFLOW_str));
    } else {
      bp_message_write_buffer(__builtin_tbladdress(MSG_UART_BAUD_ESTIMATED_str));
      bp_write_dec_dword(bit_sample);
      bp_message_write_line(__builtin_tbladdress(MSG_UART_BPS_MARKER_str));
      bp_message_write_buffer(__builtin_tbladdress(MSG_UART_BAUD_CALCULATED_str));
...
void binUART(void) {
  ...
  brg_value = UART_BRG_SPEED[0];
  uart_settings.echo_uart = 0;
  uart2_setup(brg_value, mode_configuration.high_impedance,
              uart_settings.receive_polarity, uart_settings.databits_parity,
              uart_settings.stop_bits);
  uart2_enable();
  bp_message_write_buffer(__builtin_tbladdress(MSG_UART_MODE_IDENTIFIER_str));
