...
void binOpenOCD(void) {
  ...
  bp_message_write_buffer(__builtin_tbladdress(MSG_OPENOCD_MODE_IDENTIFIER_str));
...
# 142 "../openocd.c"
    ...
    case 0x07:
      ...
      if (inByte == SERIAL_FAST) {
        user_serial_set_baud_rate(3);
      } else {
        user_serial_set_baud_rate(34);
      }
...
# 242 "../openocd.c"
      ...
      do {
        ...
        for (counter = 0; counter < bits_to_process; counter++) {
          ...
          PORTDbits.RD1 = (tdi_data_out & 0x0001) ? 1 : 0;
          tdi_data_out >>= 1;
          PORTDbits.RD4 = (tms_data_out & 0x0001) ? 1 : 0;
          tms_data_out >>= 1;
          ...
          tdo_data_in = (PORTDbits.RD3 << 15) | (tdo_data_in >> 1);
        }
...
static void binOpenOCDHandleFeature(unsigned char feat, unsigned char action) {
  switch (feat) {
  case FEATURE_LED:
    bp_set_mode_led_state(action ? 1 : 0);
    break;

  case FEATURE_VREG:
    bp_set_voltage_regulator_state(action ? 1 : 0);
    break;

  case FEATURE_PULLUP:
    bp_set_pullup_state(action ? 1 : 0);
    break;

  case FEATURE_TRST:
    PORTDbits.RD8 = action;
    break;

  case FEATURE_SRST:
    PORTDbits.RD5 = action;
    break;
