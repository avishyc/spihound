...
# 109 "../i2c.c"
static void handle_pending_ack(const _Bool bus_bit);
...
# 145 "../i2c.c"
static void hardware_i2c_send_ack(_Bool ack);
# 155 "../i2c.c"
static _Bool hardware_i2c_get_ack(void);
...
# 180 "../i2c.c"
static void i2c_sniffer(_Bool interactive_mode);
...
static _Bool i2c_write_then_read(void);

uint16_t i2c_read(void) {
  uint8_t value;

  if (i2c_state.acknowledgment_pending) {
    handle_pending_ack(0);
    user_serial_transmit_character(' ');
    bp_message_write_buffer(__builtin_tbladdress(MSG_ACK_str));
    user_serial_transmit_character(' ');
  }
  ...
  i2c_state.acknowledgment_pending = 1;
  return value;
}
...
unsigned int i2c_write(unsigned int c) {
  ...
  bp_message_write_buffer(__builtin_tbladdress(MSG_NACK_str));
  return 0x100;
}
...
void i2c_start(void) {
  ...
  bp_message_write_line(__builtin_tbladdress(MSG_I2C_START_BIT_str));
}
...
void i2c_stop(void) {
  ...
  bp_message_write_line(__builtin_tbladdress(MSG_I2C_STOP_BIT_str));
}

void i2c_print_settings(void) {
  bp_message_write_buffer(__builtin_tbladdress(BPMSG1068_str));
...
void i2c_setup_prepare(void) {
  ...
  if (speed == 0) {
    ...
    bp_message_write_line(__builtin_tbladdress(BPMSG1064_str));
    ...
    if (i2c_state.mode == 0) {
      bp_message_write_line(__builtin_tbladdress(MSG_SOFTWARE_MODE_SPEED_PROMPT_str));
      ...
      bp_message_write_buffer(__builtin_tbladdress(BPMSG1067_str));
    ...
    i2c_state.acknowledgment_pending = 0;
  }
...
void i2c_macro(unsigned int c) {
  ...
  case 0:
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1069_str));
    break;

  case 1:

    bitbang_set_pins_high(0b0000000000000010 | 0b0000000000000100, 0);
    bp_message_write_line(__builtin_tbladdress(BPMSG1070_str));

    if ((!i2c_state.to_eeprom && ((PORTDbits.RD2 == 0) || (PORTDbits.RD1 == 0))) ||
        (i2c_state.to_eeprom && ((PORTDbits.RD9 == 0) && (PORTDbits.RD10 == 0)))) {
    ...
    for (i = 0; i < 0x100; i++) {
      ...
      if (c == 0) {
        ...
        if ((i & 1) == 0) {
          bp_message_write_buffer(__builtin_tbladdress(MSG_I2C_WRITE_ADDRESS_END_str));
        } else {
          if (i2c_state.mode == 0) {
            bitbang_read_value();
            bitbang_write_bit(1);
          ...
          bp_message_write_buffer(__builtin_tbladdress(MSG_I2C_READ_ADDRESS_END_str));
        }
      }
  ...
  case 3: {
    bp_message_write_line(__builtin_tbladdress(MSG_USING_ONBOARD_I2C_EEPROM_str));

    i2c_state.to_eeprom = 1;
# 509 "../i2c.c"
    I2C1CON &=
        ~(0x00000400 | 0x00001000 | 0x00000100);
  ...
  case 4:
    if (i2c_state.to_eeprom) {
      bp_message_write_line(__builtin_tbladdress(MSG_ONBOARD_I2C_EEPROM_WRITE_PROTECT_DISABLED_str));
      PORTDbits.RD11 = 0;
    }
...
void i2c_pins_state(void) { bp_message_write_line(__builtin_tbladdress(MSG_I2C_PINS_STATE_str)); }
...
_Bool hardware_i2c_get_ack(void) {
...
void hardware_i2c_send_ack(_Bool ack) {
...
# 731 "../i2c.c"
  I2C3CON &= ~(0x00000400 | 0x00001000 | 0x00000100);
...
void i2c_sniffer(_Bool interactive_mode) {
  ...
  _Bool old_sda = PORTDbits.RD1;
  _Bool old_scl = PORTDbits.RD2;
  _Bool new_sda = old_sda;
  _Bool new_scl = old_scl;

  _Bool collect_data = 0;
  uint8_t data_bits = 0;
  ...
  for (;;) {
    ...
    new_sda = PORTDbits.RD1;
    new_scl = PORTDbits.RD2;
    ...
    if (collect_data && !old_scl && new_scl) {
      if (data_bits < 8) {
        ...
        user_serial_ringbuffer_append(new_sda ? '-'
                                              : '+');
...
void handle_pending_ack(const _Bool bus_bit) {
  if (i2c_state.mode == 0) {
...
void binary_io_enter_i2c_mode(void) {
  ...
  bp_message_write_buffer(__builtin_tbladdress(MSG_I2C_MODE_IDENTIFIER_str));
  ...
  for (;;) {
    ...
    case 0:
      ...
      case 9:
        ...
        user_serial_transmit_character(fr);
        break;
...
_Bool i2c_write_then_read(void) {
...
# 1213 "../i2c.c"
  for (size_t index = 0; index < bytes_to_read; index++) {
    ...
    bitbang_write_bit(index >= bytes_to_write ? 1 : 0);
  }
