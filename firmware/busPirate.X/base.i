...
void bp_reset_board_state(void) {
  ...
  TRISE |= 0b11100000; LATE &= (~0b11100000);
  TRISCbits.TRISC14 = 1; CNPU1 |= 0b1; CNEN1 |= 0b1; IEC1bits.CNIE = 0; IFS1bits.CNIF = 0;;
  bp_disable_3v3_pullup();
  eeprom_initialize();
...
void bp_adc_continuous_probe(void) {
  ...
  bp_message_write_line(__builtin_tbladdress(MSG_ADC_VOLTMETER_MODE_str));
...
void bp_write_formatted_integer(const uint16_t value) {
  ...
  integer = (mode_configuration.numbits < 16)
                ? (((uint16_t)value) & ((1 << (uint16_t)mode_configuration.numbits) - 1))
                : value;

  switch (bus_pirate_configuration.display_mode) {
  case HEX:
    if (mode_configuration.int16 == 1) {
...
void bp_write_bin_byte(const uint8_t value) {
  ...
  bp_message_write_buffer(__builtin_tbladdress(MSG_BINARY_NUMBER_PREFIX_str));
...
void bp_write_hex_byte(const uint8_t value) {
  bp_message_write_buffer(__builtin_tbladdress(MSG_HEXADECIMAL_NUMBER_PREFIX_str));
...
_Bool user_serial_ready_to_read(void) { return cdc_Out_len || getOutReady(); }
...
_Bool user_serial_check_overflow(void) { return 0; }

void user_serial_clear_overflow(void) {}

void user_serial_set_baud_rate(const uint16_t rate __attribute__((unused))) {}

_Bool user_serial_transmit_done(void) { return 1; }
