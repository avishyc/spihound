...
static int16_t basic_variables[26];
static int16_t basic_stack[10];
static basic_for_loop_t basic_nested_for_loops[4];
...
static char *tokens[(0xA1 - 0x80) + 1 + 1] = {
    "LET",
    "IF",
    "THEN",
    "ELSE",
...
static uint8_t basic_program_area[1024] = {0};
# 288 "../basic.c"
static _Bool search_line_number(const uint16_t line, uint16_t *result);
# 311 "../basic.c"
...
static void list(void);
# 346 "../basic.c"
static _Bool compare(char *pointer);
# 356 "../basic.c"
void handle_else_statement(void) {
  if (basic_program_area[basic_program_counter] != 0x83) {
    return;
  }

  basic_program_counter++;
  while (basic_program_area[basic_program_counter] <= 0xE0) {
    basic_program_counter++;
  }
}

_Bool search_line_number(const uint16_t line, uint16_t *result) {
  size_t index = 0;
  uint8_t token_length;
  uint16_t current_line_number;

  for (;;) {
    if (basic_program_area[index] <= 0xE0) {
      return 0;
    }
    token_length = basic_program_area[index] - 0xE0;
    current_line_number =
...
uint16_t handle_special_token(const uint8_t token) {
  ...
  case 0x98: {
    bp_enable_adc();
    uint16_t adc_measurement = bp_read_adc(1);
    bp_disable_adc();
...
int16_t get_number_or_variable(void) {
  ...
  if ((basic_program_area[basic_program_counter] == '(')) {
    ...
    if (basic_program_area[basic_program_counter] > 0x80) {
...
void interpreter(void) {
  _Bool program_counter_updated = 0;
  ...
  while (status == STATUS_CODE_UNKNOWN) {
    if (!ifstat) {
      if (basic_program_area[basic_program_counter] < 0xE0) {
        status = STATUS_CODE_PARTIAL_INSTRUCTION_ERROR;
        break;
      }

      len = basic_program_area[basic_program_counter] - 0xE0;
    ...
    case 0x81:
      ...
      if (assign()) {
        if (basic_program_area[basic_program_counter++] == 0x82) {
          ifstat = 1;
          basic_program_counter -= 3;
        }
      } else {
        while ((basic_program_area[basic_program_counter] != 0x83) &&
               (basic_program_area[basic_program_counter] <= 0xE0)) {
          basic_program_counter++;
        }
        if (basic_program_area[basic_program_counter] == 0x83) {
          ifstat = 1;
    ...
    case 0x85: {
      ...
      if (basic_current_stack_frame < 10) {
    ...
    case 0x88:
      ...
      basic_program_counter += 4;
      while ((basic_program_area[basic_program_counter] < 0xE0) &&
             (basic_program_area[basic_program_counter] != 0x83)) {
        if (basic_program_area[basic_program_counter] == '\"') {
          basic_program_counter++;
          ...
          basic_program_counter++;
                    ...
                    (basic_program_area[basic_program_counter] <= 'Z')) ||
                   ((basic_program_area[basic_program_counter] >= 0x80) &&
                    (basic_program_area[basic_program_counter] < 0xE0))) {
          temp = assign();
          bus_pirate_configuration.quiet = 0;
    ...
    case 0x8A:
      program_counter_updated = 1;
      basic_program_counter += 4;
      if (basic_current_nested_for_index < 4) {
      ...
      if (basic_program_area[basic_program_counter++] == 0x8B) {
    ...
    case 0x8D:
      ...
      if (basic_data_read_pointer == 0) {
        i = 0;
        while ((basic_program_area[i + 3] != 0x8E) &&
               (basic_program_area[i] != 0x00)) {
          i += (basic_program_area[i] - 0xE0) + 3;
        }
      ...
      if (basic_program_area[basic_data_read_pointer] > 0xE0) {
        if (basic_program_area[basic_data_read_pointer + 3] != 0x8E) {
          basic_data_read_pointer = 0;
        } else {
    ...
    case 0x9A:
      program_counter_updated = 1;
      basic_program_counter += 4;
      bp_set_voltage_regulator_state(assign() ? 1 : 0);
      handle_else_statement();
      break;
    ...
    case 0x9B:
      program_counter_updated = 1;
      basic_program_counter += 4;
      bp_set_pullup_state(assign() ? 1 : 0);
      handle_else_statement();
      break;
    ...
    program_counter_updated = 0;
  }

  bus_pirate_configuration.quiet = 0;

  if ((status != STATUS_CODE_FINISHED) && (status != STATUS_CODE_UNKNOWN)) {
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1047_str));
    bp_write_dec_word(status);
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1048_str));
    bp_write_dec_word(lineno);
    bp_message_write_buffer(__builtin_tbladdress(BPMSG1049_str));
...
void list(void) {
  ...
  bp_message_write_line(__builtin_tbladdress(BPMSG1050_str));
}

_Bool compare(char *pointer) {
...
uint8_t get_token(void) {
  ...
  for (index = 0; index < (0xA1 - 0x80) + 1; index++) {
...
void bp_basic_enter_interactive_interpreter(void) {
  ...
  if ((cmdbuf[cmdstart] >= '0') &&
    ...
    if (search_line_number(temp, &pos)) {
      len = (basic_program_area[pos] - 0xE0) + 3;

      for (i = pos; i < 1024 - len; i++) {
        basic_program_area[i] = basic_program_area[i + len];
      }
      memset(&basic_program_area[i], 0x00, 1024 - i);
    }
    ...
    while (cmdstart != cmdend) {
      ...
      i++;
      if (i > 35) {
        bp_message_write_line(__builtin_tbladdress(BPMSG1051_str));
        return;
      }
    }
    ...
    while (!end) {
      if (basic_program_area[i] > 0xE0) {
        len = basic_program_area[i] - 0xE0;
...
# 1187 "../basic.c"
      ...
      bp_message_write_line(__builtin_tbladdress(BPMSG1052_str));
    }
  }
}

void bp_basic_initialize(void) {
  basic_program_area[0] = 0xE0 + 1;
  basic_program_area[1] = 0xFF;
  basic_program_area[2] = 0xFF;
  basic_program_area[3] = 0xA1;
  memset(&basic_program_area[4], 0x00, 1024 - 4);
}
